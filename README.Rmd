...
folder. It takes a vector of one or more `teamID`s and returns selected
variables from the `Logos` table. 

Of these variables, for each `teamID`,

* `name` is the team name
* `lgID` and `divID` are the league (`AL` and `NL`) and division (`C`, `E`, `W`) ID codes.
...
In this example, I select teams in the American League to avoid too many bars.
I also reorder the `teamID`s by increasing Salary.  
```{r get-salary}
...
Get the file path of the logo image file for each team using `system.file()` to find the
`img` filenames in the `png` directory of the package.
```{r get-logos}
...
Home runs and wins. We would expect a positive relationship, and it is largely linear.
```{r W-HR}
# ------------------------------------------
...
In spite of some massaging with the `magick` package, the PNG images do not have a completely transparent background, so appear best in graphs with
a white background. You can see this effect in the bar plot above.
